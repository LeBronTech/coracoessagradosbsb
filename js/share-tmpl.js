...
(function(d, s) {
    ...
    function handleSubmit(evt) {
        ...
        if(evt.target.nodeName=="BUTTON"&&evt.target.parentElement.classList.contains('form-button')){
            ...
            if(pTar){
                    ...
                    Array.from(pTar.querySelectorAll('.form-field')).forEach(function (ele) {
                        ...
                        switch (val.key) {
                            ...
                            case 'phone':
                                ...
                                var _errSpan=d.createElement('span');
                                _errSpan.className='error iconfont icon-warn';
                                _errSpan.innerText='Please enter a valid phone number';
                                // if(!isEmpty(val.value) && !isPhone(val.value)){
                                if(!isEmpty(val.value)){
                                    try{
                                        let phoneParse=libphonenumber.parsePhoneNumber(_li.dataset.dial+' '+val.value);
                                        if(!phoneParse.isValid()){
                                            error='Please enter a valid phone number';
                                            // ele.innerHTML+='<span class="error iconfont icon-warn">{0}</span>'.Format(error);
                                            ele.appendChild(_errSpan);
                                        }
                                    }catch (e){
                                        error='Please enter a valid phone number';
                                        // ele.innerHTML+='<span class="error iconfont icon-warn">{0}</span>'.Format(error);
                ...
                if(pTar.querySelectorAll('.form-field-phone')){// filed phone 
                    if(!d.querySelector('#libphonenumber-js')){
                        var fjs=d.getElementsByTagName(s)[0];
                        var js=d.createElement(s);
                        js.id='libphonenumber-js';
                        js.async=!0;
                        // js.src='https://bio.linkcdn.cc/instabio.cc/static/vendor/jedate/jedate.js';
                        js.src='https://cdn.jsdelivr.net/npm/libphonenumber-js@1.10.50/bundle/libphonenumber-js.min.js';
                        fjs.parentNode.insertBefore(js, fjs);
                    }
                    lsdkjs(window, 'libphonenumber').then(function () {
                        _makeupFields();
                    });
                }else{
                    _makeupFields();
    ...
    function _hideAllNodesNoRelate(tarDiv){
        //33, 37, 38, 43, 44
        let pBox=eleParents(tarDiv, '.center'),tIdx=parseInt(tarDiv.dataset.idx,10);
        if(pBox){
            let nodes=pBox.querySelectorAll('[data-idx][data-type]');
            (nodes||[]).forEach(function (ele){
                if(parseInt(ele.dataset.idx,10)>tIdx){
                    if([33, 37, 38, 43, 44].includes(parseInt(ele.dataset.type, 10))){
                    }else{
    ...
    function _resetAllNodesNoRelate(tarDiv){
        //33, 37, 38, 43, 44
        let pBox=eleParents(tarDiv, '.center'),tIdx=parseInt(tarDiv.dataset.idx,10);
        if(pBox){
            let nodes=pBox.querySelectorAll('[data-idx][data-type]');
    ...
    function handlerBlockSearch(fte){
        ...
        if(fte.nodeName=='I'||fte.classList.contains('btn-icons--remove')||_input.value==''){
            ...
            }else if([33, 37, 38, 43, 44].includes(parseInt(_boxP.dataset.type, 10))){
                let items0=d.querySelectorAll('.section-home .item-goods .item-block'),
                    items1= d.querySelectorAll('.section-home .item-digital'),
                    items2= d.querySelectorAll('.section-home .item-request');
                items=Array.from(items0).concat(Array.from(items1)).concat(Array.from(items2));
            }else if(_boxP.dataset.type==1001){//1001 StorePage
                items=_boxP.parentElement.querySelectorAll('.item-block');
            }
            (items||[]).forEach(function (ele) {
                ele.classList.remove('hidden');
            })
            if(_boxP.querySelector('.no-result')) _boxP.querySelector('.no-result').remove();
            _resetAllNodesNoRelate(_boxP);
            return;
        }
        ...
        if(_fbe&&_fbe.nodeName.toUpperCase()=='SVG'||_fbe.nodeName.toUpperCase()=='INPUT'){
            ...
            _boxP = eleParents(fte, '.block-search').parentElement;
            var _noresult=true,_noNode='div',_noEle=null,searchType='';
            if(_boxP.dataset.type==10){
                items=_boxP.querySelectorAll('.button-item ');
            }else if(_boxP.dataset.type==19){
                items=_boxP.querySelectorAll('li.item');
                ...
                searchType='goods';
                if(parseInt(_boxP.dataset.type, 10)==33) _noNode='li';
                let items0=d.querySelectorAll('.section-home .item-goods .item-block'),
                    items1= d.querySelectorAll('.section-home .item-digital'),
                    items2= d.querySelectorAll('.section-home .item-request');
                items=Array.from(items0).concat(Array.from(items1)).concat(Array.from(items2));
                _hideAllNodesNoRelate(_boxP);
            }else if(_boxP.dataset.type==1001){//1001 StorePage
                searchType='goods';
                items=_boxP.parentElement.querySelectorAll('.item-block');
            }
            if(searchType=='goods'){
                (items||[]).forEach(function (ele) {
                    let txt = ele.querySelector('.text-box .title')||ele.querySelector('.support-title ')||ele.querySelector('.embed-event-title');
            ...
            if(_noresult){
                ...
                if(!_boxP.querySelector('.no-result')){
                    if(_boxP.dataset.type==10){
    ...
    function handlerSupport(fbe){
        ...
        var amountHTML='',_EP=eleParents(fbe, '.bio-support'),cy_sym=__data.ppset.cy_sym=='undefined'?(__data.ppset.currency||'$'):(__data.ppset.cy_sym||'$');
        if(_EP&&_EP.dataset.subtype=='cmpt-support-gifts'){//support gifts
            ...
            if(amount){
                _divBox.innerHTML=getTmplInnerHtml('#embed-support').Compile({amount:amountHTML,desc:txt.desc,success:decodeURIComponent(txt.success),customDisplay:txt.customAmount?'block':'none',
                    payType:_provider.type,key:_provider.key,cy_sym:cy_sym,currency:__data.ppset.currency||'USD'});
                // detail
                _divDetail.innerHTML=getTmplInnerHtml('#embed-support-detail').Compile({success:decodeURIComponent(txt.success),noteDisplay:txt.noteMessage?'block':'none',link:location.pathname.slice(1),
                    payType:_provider.type,key:_provider.key,id:_provider.id,merchantid:_provider.merchantId,cy_sym:cy_sym,currency:__data.ppset.currency||'USD'});
                _fc.style.display='block';
            ...
            txt.amount.forEach(function (val,idx) {
                amountHTML+=amountTMPL.Format(parseInt(Math.random()*10000000000,10),(val/100.00),val,cy_sym);
            });
            _divBox.innerHTML=getTmplInnerHtml('#embed-support').Compile({amount:amountHTML,desc:txt.desc,success:decodeURIComponent(txt.success),customDisplay:txt.customAmount?'block':'none',
                payType:_provider.type,key:_provider.key,cy_sym:cy_sym,currency:__data.ppset.currency||'USD'});
            // detail
    ...
    function scrollAction(pTar,actions,that){
        var _pTar=pTar.querySelector(actions[2]),len=0;
        if(actions[0]=='left'){
            // _pTar.scrollBy(-parseInt(_pTar.getBoundingClientRect().width/2,10),0);
            if(actions.length==4){
                len = _pTar.getBoundingClientRect().width/100 * parseInt(actions[3],10);
            }else {
                len = parseInt(_pTar.getBoundingClientRect().width / 2, 10);
            }
            _pTar.scrollBy({top:0,left:-len,behavior:'smooth'});
            if(_pTar.scrollLeft<=len) that.disabled=true;
        }
        if(actions[0]=='right'){
            // _pTar.scrollBy(parseInt(_pTar.getBoundingClientRect().width/2,10), 0);
            if(actions.length==4){
            ...
            _pTar.scrollBy({top:0,left:len,behavior:'smooth'});
            if(that.parentElement.querySelector('.left')) that.parentElement.querySelector('.left').disabled=false;
        }
    }
    ...
    function closePreview() {
        d.querySelector('.image_preview_bg').remove();
        d.querySelector('.previewed').classList.remove('previewed');
    }
    function imagePreviewDire(dirc) {
        var previewedImg=d.querySelector('.previewed'),src='',previewedImgLi;
        if(dirc&&previewedImg){
            previewedImgLi=ep(previewedImg,'li');
            if(dirc=='prev'&&previewedImgLi.previousElementSibling){
                src=previewedImgLi.previousElementSibling.querySelector('img').dataset.src||previewedImgLi.previousElementSibling.querySelector('img').src;
                previewedImg.classList.remove('previewed');
                previewedImgLi.previousElementSibling.querySelector('.image-box').classList.add('previewed');
            }
            if(dirc=='next'&&previewedImgLi.nextElementSibling){
                src=previewedImgLi.nextElementSibling.querySelector('img').dataset.src||previewedImgLi.nextElementSibling.querySelector('img').src;
                previewedImg.classList.remove('previewed');
                previewedImgLi.nextElementSibling.querySelector('.image-box').classList.add('previewed');
            }
            if(src) d.querySelector('.image_preview img').src=src;
        }
    }
    function viewImages(that,actions) {
        if(actions[2]=='next'||actions[2]=='prev'){
            imagePreviewDire(actions[2]);
        }else{
            var img_prev=d.querySelector('.image_preview_bg');
            that.parentElement.classList.add('previewed');
            if(!img_prev){
                img_prev=d.createElement('div');
                img_prev.className='image_preview_bg';
                d.querySelector('.container').appendChild(img_prev);
            }
            img_prev.innerHTML=getTmplInnerHtml('#tmpl-image-preview').Format('preview-list', that.querySelector('img').src);
        }
    }
    ...
    function showTmrReviewMore(pTar,actions){
        if(pTar.querySelector('.embed-tmr__box').classList.contains('showall')){
            pTar.querySelector('.embed-tmr__box').classList.remove('showall');
            pTar.querySelector('.embed-tmr__review--actions button').innerHTML=gettext('See more');
        }else{
            pTar.querySelector('.embed-tmr__box').classList.add('showall');
            pTar.querySelector('.embed-tmr__review--actions button').innerHTML=gettext('Show less');
        }
    }
    ...
    function getImageBase64(img,imgType) {
        ...
        canvas.height = img.height>320?320: img.height;
    ...
    function switchBiolinkTab(that,actions){
        let pTar=ep(that,actions[2]);
        if(pTar){
            if(!pTar.classList.contains('selected')){
                pTar.parentElement.querySelector('.selected').classList.remove('selected');
                pTar.classList.add('selected');
                if(that.dataset&&that.dataset.type==2){
                    d.querySelector('.section-home').classList.remove('selected');
                    d.querySelector('.section-store').classList.add('selected');
                    globalFN.pageStore();
                }else{
                    d.querySelector('.section-home').classList.add('selected');
                    d.querySelector('.section-store').classList.remove('selected');
                }
            }
        }
    }
    function toggleAction(toggle,action,that){
        ...
        if(toggle=='show'&&_action[0]=='more'){
            ...
            if(_action[1]=='tmr'){
                pTar=ep(that,_action[2]);
                if(pTar) showTmrReviewMore(pTar, _action);
            }
        }
        ...
        if(toggle=='close'){
            if(_action.length==3){
                ...
                }else if(_action[0]=='biolink'&&_action[1]=='view'&&_action[2]=='gallery'){
                    closePreview();
                }
            }
            return;
        ...
        if(toggle=='view'){
            if(_action.length==3){
                ...
                if(_action[0]=='biolink'&&_action[1]=='gallery'){
                    viewImages(that,_action);
                }
            }
            return;
        }
        ...
        if(toggle=='switch'){
            if(_action[0]=='biolink'&&_action[1]=='tab'){
                switchBiolinkTab(that,_action);
            }
            return;
        }
    }
    ...
    function _renderFooterCP() { 
        ...
        if(__data.ui&&__data.ui.check_state>2){
            var _report = d.createElement('div');
            _report.className = 'footer footer-report';
            _report.innerHTML = `<a href="https://www.instabio.cc/prom/report/?uri={0}" target="_blank"><i class="iconfont icon-warn1"></a>`.Format(encodeURIComponent(location.href));
            d.body.appendChild(_report);
        }
    }
