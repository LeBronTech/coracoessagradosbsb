(function (u,s) {
    ...
    var f = function (m,d) {
        ...
        // let _url='/anl/adata/link/'+__data.bio.id+'/'+(location.pathname.split('/')[1]||location.host.split('.')[0])+'/1/?'.replace('1',m);
        let _url0=__data.bio.id,_urlArr=location.pathname.split('/'),_url1,url2;
        let _url='/anl/adata/link/{0}/{1}'+'/1/?'.replace('1',m);
        if(_urlArr.length>=3){
            if(_urlArr[1]=='s'&&_urlArr[2].length>20){
                _url=_url.replace('{0}',_url0).replace('{1}',_urlArr[2]);
            }else{
                return;
            }
        }else{
            _url=_url.replace('{0}',_url0).replace('{1}',_urlArr[1]||location.host.split('.')[0]);
        }
        xhr.open('POST', anlhost+_url, true);
    ...
    var a = function (b) {
        ...
        if(b.dataset&&((_ds.html&&_ds.html!='null')||_type==10||_ds.type==10||_type==11||_ds.st=='cmpt-support-button'||_type==12||
            (_ds.st==8&&_type==1)||_type==16||_type==17||(_atype>=12))||_type==6||_type==33||_type==37||_type==38||_type==43||_type==44||_type>=1000){
            if(_ds.atype==6){
            ...
            if(_type>=1000&&_type!=1003) n=ep(b, '.ctm-style').querySelector('.info-box .title').innerText;
            if(_type==37||_type==43||_type==44){
                n=ep(b, '.ctm-style').querySelector('.embed-event-title').innerText;
            }
            if(_type==38||_type==1003){
    ...
    loaded(window).then(function () {
        let _urlArr=location.pathname.split('/');
        if(_urlArr.length>=3){
            if(_urlArr[1]=='s'&&_urlArr[2].length>20){
                f('service',['referer=' +u.referrer,'pv='+pv,'uv='+uv,'_k_sid='+k('_k_puv'),
                    'us='+q('utm_source'),'um='+q('utm_medium'),'uc='+q('utm_campaign'),
                    'l='+_urlArr[2],'t='+(10000+__data.type),
                ]);
            }
        }else{
            f('link',['referer=' +u.referrer,'pv='+pv,'uv='+uv,'_k_sid='+k('_k_puv'),
                'us='+q('utm_source'),'um='+q('utm_medium'),'uc='+q('utm_campaign'),
            ]);
        }
    });
